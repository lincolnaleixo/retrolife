...
impl LaunchConfiguration {
    pub fn normalize(mut self) -> Result<Self, String> {
        ...
        self.games.sort_by(|left, right| left.game_id.cmp(&right.game_id));
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.launchers.len() > MAX_LAUNCHERS {
            return Err(format!("launch configuration exceeds {MAX_LAUNCHERS} launchers"));
        }
...
impl SystemInputProfile {
    fn normalize(&mut self) -> Result<(), String> {
        ...
        self.bindings.sort_by_key(|binding| action_order(binding.action));
        self.validate()
    }
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        validate_identifier(
            "controller stableId",
            &self.controller.stable_id,
            128,
        )?;
        validate_text(
            "controller displayName",
            &self.controller.display_name,
            256,
        )?;
    ...
    pub fn transition(mut self, event: PrepareEvent) -> Result<Self, String> {
        ...
        match event {
            ...
            PrepareEvent::Teardown { at_ms } => {
                if matches!(self.phase, PreparePhase::Finished | PreparePhase::TearingDown) {
                    return Err(format!(
...
pub fn normalize_launch_configuration_json(json: &str) -> Result<String, String> {
    canonical_json(
        &decode_launch_configuration(json)?,
...
fn validate_template(label: &str, value: &str, maximum: usize) -> Result<(), String> {
    ...
    while let Some(start) = remaining.find('{') {
        ...
        if !matches!(
            ...
            return Err(format!("{label} uses unknown placeholder {{{placeholder}}}"));
        }
...
mod tests {
    ...
    fn missing_target_and_unknown_placeholder_are_rejected() {
        let configuration = configuration().normalize().unwrap();
        assert!(
            build_game_launch_plan(&configuration, "missing", "op-0001")
                .unwrap_err()
