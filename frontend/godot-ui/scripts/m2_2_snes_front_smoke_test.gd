...
func _run() -> void:
    ...
    _require(str(asset.get_meta("asset_id", "")) == "retrolife.snes.na-cartridge.m2.2.front.v2", "asset id")
    _require(str(asset.get_meta("prior_asset_id", "")) == "retrolife.snes.na-cartridge.m2.2.front.v1", "prior asset id")
    _require(str(asset.get_meta("source", "")) == "original-parametric-continuous-surface-rebuild", "source")
    ...
    _require(str(shell.get_meta("surface_topology", "")) == "single-connected-watertight-shell", "shell topology metadata")
    _require(int(shell.get_meta("triangle_count", 0)) > 10000, "shell triangle budget floor")
    ...
    _require(shell_size.z > 0.009 and shell_size.z < 0.011, "front depth")
    ...
    _require(uvs.size() == 4, "label UV count")
    ...
    print("RETROLIFE_M2_2_FRONT_GODOT_OK asset=v2 continuous=true watertight=true uv=true console=false physical_calibrated=false final_approval=false m2_3=false m3=false")
    quit(0)
