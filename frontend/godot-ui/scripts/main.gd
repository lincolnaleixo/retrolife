...
var _details_play_button: Button
...
func _ready() -> void:
    ...
    _gameplay = get_node_or_null("LaunchIntegration")
    if _gameplay != null and _gameplay.has_signal("game_exited"):
        _gameplay.connect("game_exited", Callable(self, "_on_game_exited"))
    ...
    var path_error := _configure_backend_paths()
    if not path_error.is_empty():
        _show_fatal_error(path_error)
        return

    _catalog = CatalogClient.new(_backend)

    if _gameplay != null and _gameplay.has_method("configure"):
        _gameplay.configure(_backend, self)

    var library_status: Dictionary = _catalog.status()
    if not bool(library_status.get("ok", false)):
        _warning_label.text = str(library_status.get("error", "Local library is unavailable."))
...
func _configure_backend_paths() -> String:
    if not _backend.has_method("configure_paths_json"):
        return "The Rust bridge does not expose the local runtime path contract."

    var user_root := OS.get_user_data_dir()
    var library_root := user_root.path_join("library")
    var save_root := user_root.path_join("saves")
    var core_path := ProjectSettings.globalize_path("res://bin/bsnes-jg_libretro.so")

    # Release builds keep the core as a real nested app resource so Rust can
    # load it with libloading. A PCK-only resource is not a valid release path.
    if OS.get_name() == "macOS":
        var executable := OS.get_executable_path()
        var contents := executable.get_base_dir().get_base_dir()
        var bundled_core := contents.path_join("Frameworks").path_join("bsnes-jg_libretro.dylib")
        if FileAccess.file_exists(bundled_core):
            core_path = bundled_core
        else:
            core_path = ProjectSettings.globalize_path("res://bin/bsnes-jg_libretro.dylib")

    var raw: Variant = _backend.call(
    ...
    if typeof(parsed) != TYPE_DICTIONARY:
        return "The Rust bridge returned invalid path configuration data."
    var response: Dictionary = parsed
    if not bool(response.get("ok", false)):
        return str(response.get("error", "The local runtime paths could not be configured."))
    return ""
...
func _build_shell() -> void:
    ...
    _import_button.text = "Import ROM"
    _import_button.tooltip_text = "Copy an .sfc or .smc file into the local library"
    _import_button.custom_minimum_size = Vector2(160, 48)
    _import_button.pressed.connect(_open_import_dialog)
    toolbar.add_child(_import_button)

    _file_dialog = FileDialog.new()
    _file_dialog.name = "RomImportDialog"
...
func _build_details_overlay() -> void:
    ...
    _details_play_button = Button.new()
    _details_play_button.name = "PlayButton"
    _details_play_button.text = "Play in RetroLife"
    _details_play_button.custom_minimum_size = Vector2(260, 52)
    _details_play_button.pressed.connect(_play_selected_game)
...
func _open_details(game_id: String) -> void:
    ...
    _details_state.text = "Ready to play with the bundled SNES core"
    if not bool(game.get("playable", true)):
        _details_state.text = "This library entry is not playable yet"
    if bool(game.get("favorite", false)):
    ...
    var playable := bool(game.get("playable", true))
    _details_play_button.disabled = not playable
    _details_play_button.visible = playable
...
func _on_files_selected(paths: PackedStringArray) -> void:
    ...
    if failures.is_empty():
        _warning_label.visible = false
        _warning_label.text = ""
        _status_label.text = "Imported %s game%s into the local library." % [
            imported,
            "" if imported == 1 else "s",
        ]
    else:
        _warning_label.text = "Import finished with %s error%s: %s" % [
            failures.size(),
...
func _play_selected_game() -> void:
    if _gameplay == null or not _gameplay.has_method("start_game"):
        _show_nonfatal_error("Gameplay is unavailable in this build.")
        return
    if _return_game_id.is_empty():
        return

    var game_id := _return_game_id
    var response: Dictionary = _catalog.details(game_id)
    if not bool(response.get("ok", false)):
        _show_nonfatal_error(str(response.get("error", "Could not load game details.")))
        return
    var game: Dictionary = response.get("data", {})
    var started: Dictionary = _gameplay.start_game(
        game_id,
        str(game.get("title", game_id))
    )
    if not bool(started.get("ok", false)):
        _show_nonfatal_error(str(started.get("error", "Could not start the game.")))
        return
    _details_overlay.visible = false


func _on_game_exited(game_id: String) -> void:
    _details_overlay.visible = false
    _status_label.text = "Session ended. RetroLife is ready."
    _last_game_id = game_id
    _focus_game_deferred(game_id)
...
func _ensure_input_actions() -> void:
    ...
    # works for keyboard and standard Godot gamepads.
    _ensure_key_action("game_up", KEY_UP)
    _ensure_key_action("game_up", KEY_W)
    _ensure_key_action("game_down", KEY_DOWN)
    _ensure_key_action("game_down", KEY_S)
    _ensure_key_action("game_left", KEY_LEFT)
    _ensure_key_action("game_left", KEY_A)
    _ensure_key_action("game_right", KEY_RIGHT)
    _ensure_key_action("game_right", KEY_D)
    _ensure_key_action("game_south", KEY_X)
    _ensure_key_action("game_east", KEY_Z)
    _ensure_key_action("game_west", KEY_C)
    _ensure_key_action("game_north", KEY_V)
    _ensure_key_action("game_left_shoulder", KEY_Q)
    _ensure_key_action("game_right_shoulder", KEY_E)
    _ensure_key_action("game_start", KEY_ENTER)
    _ensure_key_action("game_select", KEY_SHIFT)
    _ensure_key_action("game_pause", KEY_ESCAPE)

    _ensure_joy_button_action("game_south", JOY_BUTTON_A)
    _ensure_joy_button_action("game_east", JOY_BUTTON_B)
    _ensure_joy_button_action("game_west", JOY_BUTTON_X)
    _ensure_joy_button_action("game_north", JOY_BUTTON_Y)
    _ensure_joy_button_action("game_left_shoulder", JOY_BUTTON_LEFT_SHOULDER)
    _ensure_joy_button_action("game_right_shoulder", JOY_BUTTON_RIGHT_SHOULDER)
    _ensure_joy_button_action("game_start", JOY_BUTTON_START)
    _ensure_joy_button_action("game_select", JOY_BUTTON_BACK)
    _ensure_joy_button_action("game_pause", JOY_BUTTON_GUIDE)
    _ensure_joy_button_action("game_pause", JOY_BUTTON_RIGHT_STICK)
    _ensure_joy_button_action("game_up", JOY_BUTTON_DPAD_UP)
    _ensure_joy_button_action("game_down", JOY_BUTTON_DPAD_DOWN)
    _ensure_joy_button_action("game_left", JOY_BUTTON_DPAD_LEFT)
    _ensure_joy_button_action("game_right", JOY_BUTTON_DPAD_RIGHT)
    _ensure_joy_motion_action("game_left", 0, -1.0)
    _ensure_joy_motion_action("game_right", 0, 1.0)
    _ensure_joy_motion_action("game_up", 1, -1.0)
