...
**Versioned models and labels.** The public cartridge collection is the canonical source for both the neutral shell and prepared per-game carts. The initial integration target remains `snes-ntsc-u` v0.1.0 (collection commit `a5a184ff1b463b540c56a081e1da625f70067d7a`), and the prepared `snes-super-mario-world` v0.1.0 example — front label, folded top title and rear details already restored — is the first approved real per-game artifact. The collection root `catalog.json` lists each asset with its versioned manifest, and manifests carry coordinate conventions, validation counts, preview checksums and the exact release archive URL/size/SHA-256. Pin a collection commit and manifest versions; never consume floating `main` or `latest` at runtime. Download and verify during controlled preparation, or on demand with atomic caching in the app; a normal first launch must not require a 49 MB model download. Package only the required runtime resources and notices, and keep Blender sources and unrelated packages out of the app. Future asset updates are reviewed dependencies, not silent executable/content replacement.

**Asset rights and scope.** The collection declares CC BY-NC-ND 4.0 for original contributions — not the app's AGPL license — and its `snes-super-mario-world` entry explicitly excludes Nintendo artwork and marks from that license. The collection is public and owner-maintained, and the owner has directed RetroLife to consume it as the label source. Keep those terms: cartridge models and labels are fetched from the external collection (or supplied locally by the user), stay outside application source, are never relicensed under AGPL, and ship attribution/credits wherever required. Running code must not hinge on that permission: a missing, unapproved, broken or unreachable label falls back to a neutral treatment and must never prevent launching an otherwise usable game. This plan does not change the collection's license or contents.

**Collection structure for consumption.** RetroLife consumes the collection through its released catalog: stable asset IDs and slugs per system/edition/title, a versioned manifest per asset with front/top label entries (rear optional), checksums, provenance/attribution, rights notes and approval status, plus release archives under immutable tags. Keep `catalog.json` backwards compatible through its schema version. Adding a cartridge is a collection-side change — prepare the model and label package, previews, manifest and validation, open a PR there, and tag a release — so new cartridges become available to RetroLife without an application release. Document that workflow (an add-a-cartridge guide) and validate manifests in the collection's own CI. When a title exists only as a full textured GLB, derive standalone front/top label images so the app can also map them onto the shared neutral shell.
...
- [ ] Support safe per-game label textures: resolve the first real approved label from the public collection (the prepared Super Mario World package) onto the shared neutral shell, keep a basic local image override, and always provide an original title-based neutral fallback. Correct aspect ratio, crop, orientation, edge behavior, filtering and color-space handling.
...
- [ ] Consume per-game label packages from the public collection: pin a released collection commit/manifest, resolve packages by catalog identity and title with recorded provenance/confidence (never a loose filename guess), verify checksums, and download on demand through bounded atomic caching. Precedence is explicit user override, then a verified collection label, then the neutral title label; an unreachable collection degrades to neutral and never blocks browsing or play.
