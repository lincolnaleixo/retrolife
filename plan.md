...
- [ ] Generate optional Sparkle binary deltas between consecutive signed releases so an in-app update downloads only the changed bytes; verify the delta before use and fall back to the full signed ZIP when one is unavailable.
