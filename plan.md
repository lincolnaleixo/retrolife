...
**Architecture contract:** retain Rust, Godot, the GDExtension bridge, the owned libretro worker, and the pinned bundled SNES core. Do not migrate to Electron, Tauri, a web frontend, or an external emulator launcher. No account, server, runtime core download, or network connection is required to import, browse, play, or restore local progress. Network access is optional enrichment only: per-game cartridge labels may be fetched from the public cartridge collection and cached locally, and every screen must remain complete and playable offline with neutral labels.
...
| --- | --- | --- |
| macOS distribution | Public updater-enabled testing prereleases: `v0.1.0-beta.3` (2026-09-16) and `v0.1.0-beta.4` (2026-09-17), each with a signed/notarized/stapled DMG, updater ZIP, update metadata, exact corresponding source and checksums. The main-branch push that carried the ten-version plan published beta.4 automatically end to end. | Hands-on gameplay/audio/controller/save acceptance and a signed-to-signed in-app upgrade on the target Mac remain open. Completed `v0.1.0` is not established. |
...
| Frontend | `main.gd` builds text-based cards and a text-heavy details overlay. It rebuilds cards during refresh and requests the first page with `PAGE_LIMIT = 500`. | Real cartridge rendering, reusable components, deliberate layout, scalable navigation, access to the complete collection rather than only the first 500 results, a 2K-class default window, and livelier but controlled cartridge inspection. Owner review of the installed beta: the window opens small and the hero cartridge barely moves. |
...
| Cartridge collection | Public collection with `catalog.json`, per-asset versioned manifests and releases: neutral `snes-ntsc-u` v0.1.0 (22 meshes, 9 materials, embedded images, Godot 4.7.2 headless import) and `snes-super-mario-world` v0.1.0 with a restored front label, folded top title and rear details. | Application integration, actual material/surface mapping, visual verification, runtime memory measurements, and per-game labels sourced from the collection. Owner review of the installed beta: cartridges render, but only with neutral labels and barely any motion. A headless asset import is not proof of a beautiful in-app render. |
| Updates | Every `main` push is built, signed/notarized and published as a beta automatically, then the HTTPS appcast is rebuilt on the `updates` branch; beta.3 and beta.4 carry the in-app Sparkle updater. | The signed-to-signed in-app replacement, Gatekeeper acceptance and library/save preservation on the target Mac remain to be recorded. The first updater-enabled build still requires one manual install. |

The previous plan's statement that no signed release exists and its Keychain blocker are historical, not the current release status. The owner has since run the installed beta on the target Mac and reported the three gaps this revision records; gameplay/audio/controller/save acceptance and signed-to-signed update validation remain open. README, CHANGELOG and release documentation still contain older status wording; reconcile them during v0.1.0 implementation.
...
Open at a 2K-class window by default: target a 2560x1440 content area when the display can fit it; otherwise open near the full usable display area with sane margins. The installed beta opens too small and must not do so again. Keep the composition balanced at wide sizes — centered hero and controls with bounded content width instead of stretched empty panels — and remember the last window size and position (full settings persistence lands in v0.8.0).
...
The hero cartridge must feel alive and directly controllable, not a static prop. While untouched it rests in a gentle idle motion — a slow yaw sway plus a small vertical float, clearly visible but restrained — so the presentation never looks frozen. Inspection is free: pointer drag rotates through the full 360° yaw with bounded pitch (about ±60°) using damped follow and a quick settle; scroll or pinch zooms within sensible limits; double-click, a control-hint action, or Escape returns to the framed view. Keyboard and gamepad expose equivalent rotate, zoom and reset actions. Hover/focus can add a subtle lift, and selecting a game settles quickly in approximately 160-220 ms. Any momentum must be light and settle in about 300 ms or less; no forced spinning or long launch animation. Reduced motion disables idle/decorative movement and damping but never removes manual inspection, selection or launching. This living-motion requirement comes directly from owner review of the installed beta, where the cartridge barely moved.
...
**Versioned models and labels.** The public cartridge collection is the canonical source for both the neutral shell and prepared per-game carts. The initial integration target remains `snes-ntsc-u` v0.1.0 (collection commit `a5a184ff1b463b540c56a081e1da625f70067d7a`), pinned exactly for build-time staging: never consume floating `main` or `latest` when preparing or packaging application assets, verify manifest/archive SHA-256, and keep Blender sources and unrelated packages out of the app. The prepared `snes-super-mario-world` v0.1.0 example — front label, folded top title and rear details already restored — is the first owner-directed end-to-end target for the runtime label path, with its rights scope governed by the collection's own notices. The collection root `catalog.json` lists each asset with its versioned manifest; manifests carry coordinate conventions, validation counts, preview checksums and the exact release archive URL/size/SHA-256. Runtime label fetching follows the dedicated consumption model below and never changes what is bundled. A normal first launch must not require a 49 MB model download. Future asset updates are reviewed dependencies, not silent executable/content replacement.

**Asset rights and scope.** The collection declares CC BY-NC-ND 4.0 for original contributions — not the app's AGPL license — and its `snes-super-mario-world` entry explicitly excludes Nintendo artwork and marks from that license. Maintaining the collection grants no rights to third-party artwork, and RetroLife must not claim or imply one: Nintendo artwork and other excluded material are never bundled into application source, builds, release artifacts or public evidence, and `THIRD_PARTY_NOTICES.md` keeps stating that no additional grant is inferred. The owner has directed RetroLife to integrate the collection as an optional per-game label source for users' own machines; fetched labels keep the collection's attribution and rights notes, are treated like user-supplied labels in policy terms, and remain outside application source. The Super Mario World package is the first end-to-end integration fixture on the owner's machine — not a redistributable game-art pack. A missing, unapproved, broken or unreachable label falls back to the neutral treatment and never prevents launching an otherwise usable game. This plan does not change the collection's license, contents or notices.

**Collection structure for consumption.** RetroLife consumes the collection through its released catalog over HTTPS from the fixed repository, under two distinct trust modes. Build-time assets are pinned exactly, through the asset lock and manifest/archive SHA-256. Runtime labels are data, not executable content: the app follows the repository's published catalog, verifies every package against its manifest checksums before atomic caching, records the catalog/manifest revision and package hash it used, and never silently replaces a verified cached package — refreshes are explicit or reviewed, and a bad or unreachable catalog degrades to the local cache or neutral labels. That keeps new cartridges publishable without an application release while bundled assets stay frozen. Keep `catalog.json` backwards compatible through its schema version, validate manifests in the collection's own CI, and keep per-asset entries machine-readable: stable IDs, system/edition/title slugs, front/top label entries (rear optional), checksums, provenance/attribution, rights notes and approval status. Adding a cartridge stays a collection-side change with a documented add-a-cartridge guide; when a title exists only as a full textured GLB, derive standalone front/top label images for the shared neutral shell.
...
**Acceptance:** show the real model inside the Mac app in hero, front, top, rear and close label views. No distorted shell, clipped edges, mirrored/stretched label, z-fighting, floating folded label or unintended transparency. Two different locally labeled games can be inspected successively without label bleed. The hero visibly moves at idle and can be rotated and zoomed freely with a predictable reset. Super Mario World shows its real prepared label rather than a neutral one in owner-side validation, while public evidence stays artwork-free unless the asset rights review allows it, and a neutral-label game still looks intentional and can launch. Compare against the asset's reference renders and inspect the exported build, not only the editor.

**Primary areas:** new cartridge presentation resources under `frontend/godot-ui`, an asset manifest/build adapter with label-package consumption from the cartridge collection, existing build/export scripts, and asset notices. Geometry changes, if needed, belong to a separately versioned upstream model change, not an untracked copy.
...
- [ ] Separate browse navigation, inspection gestures, dialogs and gameplay input contexts, including gamepad rotate/zoom/reset while inspecting. Test trackpad scrolling, repeated key/gamepad input and controller disconnection during selection.
...
- [ ] Ship a portable library backup: export and restore metadata, SRAM and labels as a checksum-validated archive, with a dry-run/merge preview, non-destructive restore and optional ROM inclusion that defaults to off. The archive schema is explicitly versioned so state files join the backup once the v0.7 format exists, with a safe refusal of unknown future sections.
...
**Primary areas:** `crates/retrolife-core`, `crates/retrolife-library`, bridge contracts, game-details/label-editing UI, cartridge texture/cache logic, and the cartridge collection's catalog/manifest schema.
...
- [ ] Generate optional Sparkle binary deltas between consecutive signed releases so an in-app update downloads only the changed bytes; verify the delta before use and fall back to the full signed ZIP when one is unavailable. Pipeline implemented (workflow, feed, audit and helper delta step staged on the signing Mac); activation awaits the one-time host-helper install.
...
These are proposed acceptance targets, not benchmarks already achieved. Establish the reproducible baseline during v0.2-v0.4 on a documented Apple Silicon Mac, preferably an M1-class 8 GB reference machine with local SSD; measure the baseline budget at a 1280x720 logical viewport and additionally at the 2K-class default window on the target display. Record the renderer and display configuration. Report cold and warm measurements separately.
...
- [ ] Exercise the complete 3D library natively on macOS with keyboard, physical controller and mouse, and address the owner-reviewed gaps recorded above (2K-class default window, visibly livelier inspection, real collection labels), then record performance and acceptance evidence before claiming the milestone complete. Hosted Apple Silicon headless tests do not close this hands-on gate.
...
- [ ] Validate the signed-to-signed in-app replacement on the target Mac: beta.3 is the first manually installed updater-enabled build and the feed already offers beta.4. Record Gatekeeper acceptance, installed identity and library/save preservation after the in-app upgrade.

Beta.3 and beta.4 are updater-enabled, and the appcast publishes automatically on every release. Existing beta.1 and beta.2 have no updater; they require a one-time manual installation of the first new updater-enabled signed build and remain immutable.
...
Repository status and implementation: [architecture](docs/architecture.md), [release instructions](docs/releases.md), [development rules](rules.md), [published beta.4](https://github.com/lincolnaleixo/retrolife/releases/tag/v0.1.0-beta.4), [frontend shell](frontend/godot-ui/scripts/main.gd), [managed library](crates/retrolife-library/src/lib.rs), [emulation worker](crates/retrolife-emulation/src/lib.rs), and [source-release workflow](.github/workflows/release.yml).

Creative dependency: [cartridge collection](https://github.com/lincolnaleixo/retro-cartridge-models), its [catalog](https://github.com/lincolnaleixo/retro-cartridge-models/blob/main/catalog.json), the [pinned neutral-model manifest](https://github.com/lincolnaleixo/retro-cartridge-models/blob/a5a184ff1b463b540c56a081e1da625f70067d7a/assets/snes-ntsc-u/versions/0.1.0.json), and the collection's license, credits and notices. Refer to the manifests for exact package/GLB checksums rather than trusting a filename.
### In-app updater
...
Still required: owner visual review, sustained rendering/performance on the target Mac, physical controller and audible output, user-game save restoration, and the reviewed window/motion/label gaps. The cartridge interface first shipped to owners in the updater-enabled beta.3 and beta.4; beta.1 and beta.2 do not contain it.
