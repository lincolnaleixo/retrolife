...
- Keep the two long-lived branches: protected `main` and the machine-managed `updates` distribution feed. Never delete, rewrite or merge `updates`; short-lived `feat/`, `fix/` and `docs/` branches are expected and are deleted after merge.
