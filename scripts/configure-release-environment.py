...
def main() -> None:
    ...
    app_password = getpass.getpass("Apple app-specific password for notarization: ")
    if not certificate_password or not app_password:
    ...
    with tempfile.TemporaryDirectory(prefix="retrolife-key-transfer-") as temporary:
        ...
        values = {"MACOS_CERTIFICATE_P12_BASE64": base64.b64encode(path.read_bytes()).decode(),
                  "MACOS_CERTIFICATE_PASSWORD": certificate_password, "APPLE_ID": args.apple_id,
                  "APPLE_APP_PASSWORD": app_password, "SPARKLE_PRIVATE_KEY": private_value}
